...
public class Buffer {
  ...
  public Integer findMinIdCustomer(){
    ...
    for (Order order : orders) {
      if (order != null && minIdCustomer > order.getClientId()) {
        minIdCustomer = order.getClientId();
      }
    }
...
//      makeStatistics(canceledOrder, order);
  ...
  public Order getOrder() {
    ...
    if (isElementInQueueToDelete()) {
      ...
      for (int i = 0; i < orders.size(); i++) {
        if (orders.get(i) != null && orders.get(i).getClientId() == minIdCustomer) {
  ...
  private void makeStatistics(@NotNull final Order canceledOrder, @NotNull final Order order) {
    Controller.statisticsPub.orderCanceled(canceledOrder.getClientId(),
