  ...
  public void actionPerformed(ActionEvent e) {
    ...
    for (int i = 0; i < controller.getBuffer().getCapacity(); i++) {
      ...
      if (order == null) {
        ...
        if (controller.getBuffer().getElementsToDelete()[i]){
          bufferTable.setValueAt(order.getOrderId() + " package", i, 1);
        } else {
          bufferTable.setValueAt(order.getOrderId(), i, 1);
        }
      }
    }
    // bufferTable.setValueAt("~~>  ", controller.getBuffer().getInsertIndex(), 1);

    canselTable.setValueAt(controller.getBuffer().getCanceledOrder().getOrderId(), 0,0);
    ...
    for (int i = 0; i < controller.getDevices().size(); i++) {
      ...
      if (order == null) {
        ...
        devicesTable.setValueAt(order.getOrderId(), i, 1);
      }
    }
    String element;
    if (event.eventType == Type.GENERATED) {
      ...
      event.setOrderId(controller.getBuffer().getOrders().get(controller.getBuffer().getSize()-1).getOrderId());
      element = "User №" + event.id;
    } else {
      element = "Device №" + event.id;
    }
    if (event.id == -1) {
      if (controller.getBuffer().isFull()) {
        ...
        element = "All device is busy";
        event.setEventType(Type.WARNING);
      }
    }

    if (controller.getBuffer().getCanceledOrder().getOrderId().equals("-")) {
      ...
      resultsTable.addRow(new Object[]{
              ...
              element,
              event.eventType,
              controller.getBuffer().getCanceledOrder().getOrderId(),
              controller.getStatistics().getCompletedOrdersCount(),
              controller.getStatistics().getCanceledOrdersCount()});
    }
